...
module.exports.getSetting = async (domain, token, appid, target) => {
  const { body } = await request(
    ...
    )}&fields[0]=${encodeURIComponent('appid')}&fields[1]=${encodeURIComponent('token')}&fields[2]=${encodeURIComponent(
      'appSecret',
    )}&fields[4]=${encodeURIComponent('appKey')}&fields[5]=${encodeURIComponent('callback')}`,
    {
      headers: {
        'X-Cybozu-API-Token': token,
      },
